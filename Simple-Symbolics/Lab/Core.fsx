#I @"C:\Users\Admin\Code\Libs\MathNet"
#I @"C:\Users\Admin\Code\Libs\net4+"
...
let factorsExpr =
    abs >> factors (fun (i: Expression) -> i.ToInt()) (Expression.FromInt32)
...
type Units(q: Expression,u: Expression,?altUnit) =
    ...
    static member (+)(a: Units,b: Units) =
        if a.Unit = b.Unit then Units(a.Quantity + b.Quantity,a.Unit,a.AltUnit)
    ...
    static member (*)(a: Units,b: Units) =
        Units
            (a.Quantity * b.Quantity,a.Unit * b.Unit,a.AltUnit + " " + b.AltUnit)
    ...
    static member To(a: Units,b: Units,unitstr) =
        if a.Unit = b.Unit then 
...
module Units =
    ...
    let simplifyUnits(u: Units) =
        ...
        |> List.map
               (fun (u',t) -> 
               Units.To(u,u') |> Option.map(fun s -> s + " (" + t + ")"))
